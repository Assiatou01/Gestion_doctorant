                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-4">
                            ...
                            <select name="faculte.id" class="form-select">
                                ...
                                <c:forEach items="${facultes}" var="fac"><option value="${fac.id}" ${doctorant.faculte != null && doctorant.faculte.id == fac.id ? 'selected' : ''}>${fac.nom}</option></c:forEach>
                            </select>
                        </div>
                        <div class="col-md-4">
                            ...
                            <select name="laboratoire.id" class="form-select">
                                <option value="">-- Choisir --</option>
                                <c:forEach items="${laboratoires}" var="lab"><option value="${lab.id}" ${doctorant.laboratoire != null && doctorant.laboratoire.id == lab.id ? 'selected' : ''}>${lab.nom}</option></c:forEach>
                            </select>
                        </div>
                        <div class="col-md-4"><label class="form-label fw-semibold">Niveau / Maturation</label><input type="text" name="maturation" class="form-control" value="${doctorant.maturation}"/></div>
                    </div>
                    <div class="row g-3 mb-3">
